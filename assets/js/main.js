...
class DriizzyBoosts {
    ...
    showOrderModal(packageName, price) {
        ...
        modal.querySelector('#order-modal-submit').onclick = async () => {
            ...
            this.createOrUpdateCustomerAccount(orderData);
...
DriizzyBoosts.prototype.storeOrderLocally = function(orderData) {
    try {
        ...
        console.log('✅ Order stored locally:', orderData.orderId);
    } catch (error) {
        console.error('❌ Failed to store order locally:', error);
    }
};

DriizzyBoosts.prototype.createOrUpdateCustomerAccount = function(orderData) {
    try {
        // This method ensures customer account data is available for the account system
        const customerData = {
        ...
        // Store customer info for account lookup
        const existingCustomers = JSON.parse(localStorage.getItem('driizzyy_customers') || '[]');
        const existingIndex = existingCustomers.findIndex(c => 
            c.discord === customerData.discord || c.email === customerData.email
        );
        
        if (existingIndex !== -1) {
            // Update existing customer
            existingCustomers[existingIndex].lastOrderDate = customerData.lastOrderDate;
            existingCustomers[existingIndex].orderId = customerData.orderId; // Update to latest order ID for login
        } else {
            // Add new customer
            existingCustomers.push(customerData);
        }
        
        localStorage.setItem('driizzyy_customers', JSON.stringify(existingCustomers));
        console.log('✅ Customer account updated:', customerData.discord);
    } catch (error) {
