...
class DriizzyBoosts {
    ...
    setupShopFunctionality() {
        // Attach handler to all product buttons (static and dynamic)
        document.querySelectorAll('.order-btn, .pricing-card .btn').forEach(btn => {
            btn.onclick = (e) => {
                e.preventDefault();
                const card = btn.closest('.pricing-card');
                const packageName = card ? card.querySelector('h4').textContent : btn.dataset.package;
                const price = card ? card.querySelector('.amount').textContent : btn.dataset.price;
                this.showOrderModal(packageName, price);
            };
        });
        window.driizzyBoosts = this;
    }

    showOrderModal(packageName, price) {
        // Remove any existing modal
        let modal = document.getElementById('custom-order-modal');
        ...
        modal.style.cssText = `
            ...
            z-index: 9999;`;
        modal.innerHTML = `
            <div style="background: #23272a; padding: 2rem 2.5rem; border-radius: 16px; min-width: 320px; max-width: 90vw; box-shadow: 0 8px 32px #000a; position: relative;">
                <button id="order-modal-close" style="position:absolute;top:12px;right:12px;background:none;border:none;font-size:1.5rem;color:#fff;cursor:pointer;">&times;</button>
                <h2 style="margin-bottom:1rem;">Order: <span style='color:#7289da;'>${packageName}</span></h2>
                <div style="margin-bottom:1rem; font-size:1.1rem; color:#b9bbbe;">Price: <b>$${price}</b></div>
                <input id="order-discord" type="text" placeholder="Your Discord Username (e.g. user#1234)" style="width:100%;margin-bottom:0.75rem;padding:0.5rem;border-radius:6px;border:1px solid #444;background:#2c2f33;color:#fff;" required />
                <input id="order-email" type="email" placeholder="Your Email (for receipt)" style="width:100%;margin-bottom:0.75rem;padding:0.5rem;border-radius:6px;border:1px solid #444;background:#2c2f33;color:#fff;" required />
                <div id="order-modal-status" style="min-height:1.5em;margin-bottom:0.5rem;font-size:0.98rem;"></div>
                <button id="order-modal-submit" style="width:100%;padding:0.7rem 0;border-radius:8px;background:#5865f2;color:#fff;font-weight:600;font-size:1.1rem;border:none;cursor:pointer;">Submit Order</button>
            </div>
        `;
        ...
        // Submit order
        modal.querySelector('#order-modal-submit').onclick = async () => {
            const discord = modal.querySelector('#order-discord').value.trim();
            const email = modal.querySelector('#order-email').value.trim();
            const statusDiv = modal.querySelector('#order-modal-status');
            
            if (!discord || !email) {
                statusDiv.textContent = 'Please enter your Discord username and email.';
            ...
            // Get webhook and ticket channel from config
            let webhook = '', ticket = '';
            if (this.siteData && this.siteData.shop) {
                webhook = this.siteData.shop.webhook_url;
                ticket = this.siteData.shop.ticket_channel;
            }
            if (!webhook) {
                statusDiv.textContent = 'Order system unavailable. Please contact support.';
                statusDiv.style.color = 'red';
                return;
            }
            ...
            const payload = {
                ...
                embeds: [{
                    ...
                    footer: {
                        text: `Order System • ${new Date().toLocaleString()}`
                    },
                    timestamp: new Date().toISOString()
                }]
            };
            ...
            try {
                ...
                statusDiv.innerHTML = `✅ Order created!<br><strong>Order ID: ${orderId}</strong><br>Redirecting to support...`;
                statusDiv.style.color = 'green';
                setTimeout(() => {
                    window.location.href = ticket || 'https://discord.com';
                }, 1200);
            } catch (err) {
                statusDiv.textContent = 'Failed to send order. Please try again.';
                statusDiv.style.color = 'red';
            }
        };
    }
    ...
    setupPerformanceOptimizations() {
                ...
                document.querySelectorAll('section, .tab-content, .brand-showcase').forEach(el => {
                    observer.observe(el);
                });
            ...
            handleScroll() {
                ...
                // Parallax effect for brand showcase
                const brandShowcase = document.querySelector('.brand-showcase');
                if (brandShowcase) {
                    const rect = brandShowcase.getBoundingClientRect();
                    if (rect.top < window.innerHeight && rect.bottom > 0) {
                        const scrolled = window.pageYOffset;
                        const parallax = scrolled * 0.2;
                        brandShowcase.style.transform = `translateY(${parallax}px)`;
                    }
                }
            }
    ...
    createAnnouncementBanner() {
        if (!this.siteData.announcements || !this.siteData.announcements.enabled) {
            return;
        }

        const announcement = this.siteData.announcements;
        
        // Remove existing announcement if any
        const existingBanner = document.querySelector('.announcement-banner');
        if (existingBanner) {
            existingBanner.remove();
        }

        // Create enhanced announcement banner
        const banner = document.createElement('div');
        banner.className = `announcement-banner announcement-${announcement.type || 'info'}`;
        
        banner.innerHTML = `
            <div class="announcement-decorative"></div>
            <div class="announcement-content">
                <div class="announcement-left">
                    ...
                    <div class="announcement-text">
                        <div class="announcement-title">${announcement.message}</div>
                ...
                <div class="announcement-right">
                    <a href="#products" class="announcement-cta">
                        <i class="fas fa-shopping-cart"></i> Shop Now
                    </a>
                    <button class="announcement-close" onclick="this.parentElement.parentElement.parentElement.style.display='none'">
                        ×
                    </button>
                </div>
            </div>
        ...
        // Insert after navbar but before hero section
    ...
    // ===== POPULATE TAB PRODUCTS ===== //
    populateTabProducts() {
        if (!this.siteData.services) {
            console.error('❌ No services data found in siteData');
            return;
        }

        console.log('🔧 Populating tabs with services:', Object.keys(this.siteData.services));

        // Populate each tab with its products
        this.populateProductTab('server-boosts', this.siteData.services.server_boosts);
        this.populateProductTab('user-nitro', this.siteData.services.user_nitro);
        this.populateProductTab('tokens', this.siteData.services.tokens);
        this.populateProductTab('member-boosts', this.siteData.services.member_boosts);
        this.populateProductTab('boost-bot', this.siteData.services.boost_bot);
        this.populateProductTab('bot-hosting', this.siteData.services.bot_hosting);
    }

    // ===== POPULATE INDIVIDUAL PRODUCT TAB ===== //
    populateProductTab(tabId, products) {
        const grid = document.getElementById(`${tabId}-grid`);
        if (!grid) {
            console.error(`❌ Grid not found for tab: ${tabId}`);
            return;
        }
        
        if (!products) {
            console.warn(`⚠️  No products found for tab: ${tabId}`);
            return;
        }

        console.log(`🔧 Populating ${tabId} with ${products.length} products`);

        grid.innerHTML = '';

        products.forEach(product => {
            const card = this.createProductCard(product, tabId);
    ...
    createProductCard(product, category) {
        ...
        switch(category) {
            case 'tokens':
                categoryClass = 'token-card';
                break;
            case 'member-boosts':
                categoryClass = 'member-card';
                break;
            case 'boost-bot':
                categoryClass = 'bot-card';
                break;
            case 'bot-hosting':
                categoryClass = 'hosting-card';
                break;
        }
        
        card.className = `pricing-card ${product.popular ? 'featured' : ''} ${categoryClass}`;

        const popularBadge = product.popular ? 
            `<div class="popular-badge">${product.popular === true ? 'Most Popular' : 'Popular'}</div>` : '';

        // Determine price period text
        let periodText = 'one-time';
        if (product.type === 'gift_code') periodText = 'gift code';
        else if (product.type === 'bot_subscription') periodText = product.duration;
        else if (product.type === 'hosting_service') periodText = 'monthly';
        else if (product.type === 'account_token') periodText = 'each';
        else if (product.type === 'member_package') periodText = product.duration;
        else if (product.duration === 'lifetime') periodText = 'lifetime';
        else if (product.duration === 'permanent') periodText = 'permanent';
        ...
        card.innerHTML = `
            ...
            <button class="btn btn-primary order-btn" data-package="${product.name}" data-price="${product.price}">Order Now</button>
        `;
    ...
    orderService(serviceName, price) {
        this.openCheckoutModal(serviceName, price);
    }
    ...
    initializeLoadingStates() {
        // Only attach loading state to .btn inside forms (not modal finalize or pricing buttons)
...
// Add helper methods to DriizzyBoosts class
DriizzyBoosts.prototype.generateOrderId = function() {
    const timestamp = Date.now().toString();
    const random = Math.random().toString(36).substring(2, 8).toUpperCase();
    return `DB-${timestamp.slice(-6)}${random}`;
};

// Make openOrderModal accessible globally
DriizzyBoosts.prototype.openOrderModal = function(packageName, price) {
    this.showOrderModal(packageName, price);
};
